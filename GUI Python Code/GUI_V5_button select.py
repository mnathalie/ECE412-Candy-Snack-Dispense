...
class SettingsPage(tk.Frame):
    ...
    Rad1 = tk.Radiobutton(parent, text="SMALL", value='small', variable=option)
    Rad1.pack()
    Rad2 = tk.Radiobutton(parent, text="MEDIUM", value='medium', variable=option)
    Rad2.pack()
    button = tk.Button(parent, text="Save", command=validate)
    button.pack()
        #Label widget
    label = tk.Label(parent, text="Setting", font=LARGE_FONT)
        #.pack organizes widget in blocks using x and y axis
    label.pack(pady=10,padx=10)

    label = tk.Label(parent, text="Select Amount of Candy to Dispense", font=SMALL_FONT)
    label.pack(pady=10,padx=10)
