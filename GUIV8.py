...
class Password(ttk.Frame):                                         #Password Page                   
    def __init__(self, parent, controller):
        ...
        self.Username_entry = ttk.Entry(self).grid(row=0, column=1)
        ttk.Label(self, text='Password', font=SMALL_FONT).grid(row=1)   #Label for Password 
        self.Password_entry = ttk.Entry(self).grid(row=1, column=1)
    ...
    def Login_Button_Clicked(self):
        self.controller.show_frame(PageThree)#this should be commented when you run but for know leave it uncommected so you can see how it will run
        
        if self.Username == self.Username_entry.get() and self.Password == self.Password_entry.get():
